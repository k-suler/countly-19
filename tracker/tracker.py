...
def load_model():
    ...
    # trained model
    #cv2.dnn.readNet('models/faster-rcnn-person.pt')
    return cv2.dnn.readNetFromCaffe('models/tracker_model.prototxt',
                                    'models/tracker_model.caffemodel')
...
def send_to_firebase(current_time, count):
    ...
    store_id = ref.child(config.store_id)
...
while True:
    ...
    if total_frames % config.skip_frames == 0:
        ...
        for tracker in trackers:
            ...
            # update the tracker and use current position
            tracker.update(rgb)
        ...
        if tracked_person is None:
            ...
            if not tracked_person.is_counted:
                ...
                if direction < 0 and position[1] < entrance:
                    ...
                    send_to_firebase(current_time=int(time.time() * 1000), count=-1)
